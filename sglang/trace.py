...
def log_metrics(metric_file):
    metrics = get_metrics()
    # Write to file
    metric_file = open(f'./results/{metric_file}','w')
...
def validation(responses, validation_file, trace_file, sglang_to_apex_file):
    ...
    prompt_len_sglang_sum = 0
    output_len_sglang_sum = 0
    ...
    for response,request in responses:
        ...
        completion_file = open(f'./results/{validation_file}','a')
        ...
        prompt_len_sglang_sum += response.usage.prompt_tokens
        output_len_sglang_sum += response.usage.completion_tokens
    ...
    with open(f'../traces/sglang_to_apex_traces/{sglang_to_apex_file}', 'w') as file:
        for item in reqs:
    ...
    sglang_prompt_line = (f'sglang prompt length sum: {prompt_len_sglang_sum}\n')
    sglang_output_line = (f'sglang output len sum: {output_len_sglang_sum}\n')

    prompt_diff_line = (f'Total prompt len difference(sglang- trace): {prompt_len_sglang_sum - prompt_len_trace_sum}\n')
    output_len_diff_sum = (f'Total output len difference(sglang - trace): {output_len_sglang_sum - output_len_trace_sum }\n')

    completion_file.write(trace_prompt_line)
    completion_file.write(trace_output_line)

    completion_file.write(sglang_prompt_line)
    completion_file.write(sglang_output_line)
...
async def main(requests, validation_file, trace_file, sglang_to_apex_file):
    ...
    validation(flat_responses, validation_file, trace_file, sglang_to_apex_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trace-file", type=str, default="../traces/simplified_irregular_trace.jsonl")
    ...
    asyncio.run(main(requests, args.validation_file, args.trace_file, args.sglang_to_apex_file))
    
    # Log metrics to designated file
    avg_ttft, avg_tpot, avg_latency = log_metrics(args.metric_file)

    # Write Validation file
    completion_file = open(f'./results/{args.validation_file}','a')
    ttft_line = (f'sglang TTFT: {avg_ttft} \n')
    tpot_line = (f'sglangTPOT: {avg_tpot} \n')
    latency_line = (f'sglang Total Latency: {avg_latency} \n')
